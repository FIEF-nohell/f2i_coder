...
for file in files:
    ...
    extension_bitstring = ''.join(format(ord(c), '08b') for c in file)
    root = math.ceil(math.sqrt(len(bitstring_from_file)+len(extension_bitstring)))
    width, height = root, root
    image_data = [None] * len(bitstring_from_file)
    ...
    num_iterations = len(bitstring_from_file)
    ...
    for i in range(num_threads):
        ...
        thread = threading.Thread(target=process_bits, args=(bitstring_from_file, start, end, image_data, i))
        thread.start()
...
print("All files have been encoded.\n\n")